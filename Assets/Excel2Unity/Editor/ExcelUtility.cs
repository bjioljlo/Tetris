...
public class ExcelUtility
{
	...
	public void ConvertToCSV (string CSVPath, Encoding encoding)
	{
		...
		{
			//判断数据表内是否存在数据
			if (child.Rows.Count < 1)
                return;

            //读取数据表行数和列数
			int rowCount = child.Rows.Count;
			int colCount = child.Columns.Count;
            ...
            for (int i = 0; i < rowCount; i++)
            {
                ...
                {
                    //使用","分割每一个数值
					stringBuilder.Append(child.Rows[i][j] + ",");
                }
                //使用换行符分割每一行
                stringBuilder.Append("\r\n");
            }

			string mCSVPath = ReSetPath(CSVPath,child.TableName);
            //写入文件
			using (FileStream fileStream = new FileStream(mCSVPath, FileMode.Create, FileAccess.Write))
            {
                using (TextWriter textWriter = new StreamWriter(fileStream, encoding))
                {
	...
	{
		string str_Temp = oldPath;

		int pos_slash, pos_dot;
		//pos_slash = str_Temp.LastIndexOf("/");
		pos_dot = str_Temp.LastIndexOf(".");
		if(pos_dot < 0)
		{
			return null;
		}
		return str_Temp.Insert(pos_dot, "_" + sheetName);
